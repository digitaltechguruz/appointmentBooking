...
/** Shown first in the country picker for quicker access. */
export const POPULAR_PHONE_COUNTRY_ISOS = [
  "US",
  "CA",
  "GB",
  "AU",
...
export const PHONE_COUNTRIES: PhoneCountry[] = [
  ...
  { iso: "VG", name: "Virgin Islands (British)", dial: "+1" },
  { iso: "VI", name: "Virgin Islands (U.S.)", dial: "+1" },
  { iso: "YE", name: "Yemen", dial: "+967" },
  { iso: "ZM", name: "Zambia", dial: "+260" },
  { iso: "ZW", name: "Zimbabwe", dial: "+263" },
];

const phoneCountryByIso = new Map(
...
export function getPhoneCountry(iso: string): PhoneCountry | undefined {
  return phoneCountryByIso.get(iso);
}

export function getPhoneCountryDial(iso: string): string {
  return getPhoneCountry(iso)?.dial ?? "+1";
}

export function formatPhoneCountryLabel(country: PhoneCountry): string {
  return `${flagEmoji(country.iso)} ${country.dial} ${country.name}`;
}

export function detectDefaultPhoneCountryIso(): string {
  ...
  for (const language of languages) {
    const match = language.match(/[-_](?<region>[A-Z]{2})\b/i);
    const region = match?.groups?.region?.toUpperCase();
    if (region && phoneCountryByIso.has(region)) {
      return region;
    }
  }
...
export function listPhoneCountriesGrouped() {
  const popularSet = new Set<string>(POPULAR_PHONE_COUNTRY_ISOS);
  const popular = POPULAR_PHONE_COUNTRY_ISOS.map((iso) =>
    phoneCountryByIso.get(iso),
  ).filter((country): country is PhoneCountry => Boolean(country));

  const all = [...PHONE_COUNTRIES].sort((a, b) =>
    a.name.localeCompare(b.name, "en"),
  );

  return { popular, all, popularSet };
}
